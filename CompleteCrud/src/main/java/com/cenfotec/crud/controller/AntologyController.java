...
public class AntologyController {
	...
	public String showUpdateForm(@PathVariable("id") long id, Model model) {
		Anthology anthology = AntologyRepository.findById(id)
		.orElseThrow(() -> new IllegalArgumentException("Invalid anthology Id:" + id));
    
		model.addAttribute("antology", antology);
		return "update-antology";
	}
	
	AntologyRepository.save(antology);
    model.addAttribute("anthologies", userRepository.findAll());
